...
const CACHE_NAME  = 'minikuyku-v2.2';
const AUDIO_CACHE = 'minikuyku-audio-v2.2';
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(k => k !== 'minikuyku-v2.2' && k !== 'minikuyku-audio-v2.2').map(k => caches.delete(k))))
