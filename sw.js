...
const CACHE_NAME  = 'minikuyku-v2.3';
const AUDIO_CACHE = 'minikuyku-audio-v2.3';
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(k => k !== 'minikuyku-v2.3' && k !== 'minikuyku-audio-v2.3').map(k => caches.delete(k))))
