...
window.injectBabyName = function(text) {
  ...
  return text
    .replace(/bebeğim/gi, state.babyName + "'m")
...
window.generateSpeech = async function(text, trackId, page) {
  ...
  try {
    const res = await fetch(
      'https://api.elevenlabs.io/v1/text-to-speech/' + CONFIG.DEFAULT_VOICE_ID + '/stream',
      {
        method: 'POST',
...
window.playVoiceLullaby = async function(id) {
  if (state.currentTrack === 'v'+id && state.isPlaying) { await stopAll(); return; }
  const lullaby = getActiveLullabies().find(l => l.id === id);
  if (!lullaby) return;
  await stopAll();
...
window.cloneVoice = async function() {
  ...
  if (!CONFIG.ELEVENLABS_API_KEY) {
    ...
    const vg = document.getElementById('voice-generator');
  ...
  try {
    ...
    const res  = await fetch('https://api.elevenlabs.io/v1/voices/add', {
      method:'POST', headers:{'xi-api-key': CONFIG.ELEVENLABS_API_KEY}, body: fd
    });
    ...
    showStatus('clone-status','error', (state.language==='tr'?'Hata: ':'Error: ') + e.message);
  } finally {
    if (btn) btn.disabled = false;
  }
};
...
window.speakWithClonedVoice = async function() {
  ...
  try {
    ...
    showStatus('speak-status','error', (state.language==='tr'?'Hata: ':'Error: ') + e.message);
  } finally {
    if (btn) btn.disabled = false;
  }
};
...
function generateKolikSound(id) {
  ...
  switch(id) {
    case 'b1': _noise(master,'pink',800);   break;
    case 'b2': _noise(master,'brown',400);  break;
    case 'b3': _noise(master,'white',1000); break;
    case 'b4': _noise(master,'pink',200);   break;
    case 'd1': _noise(master,'pink',800);   break;
    case 'd2': _wave(master);               break;
    case 'd3': _forest(master);             break;
    case 'd4': _noise(master,'white',2000); break;
    case 'r1': _heartbeat(master);          break;
    case 'r2': _pispis(master);             break;
  }
}
...
function _noise(dest, type, freq) {
  ...
  src.connect(flt); flt.connect(dest); src.start();
  state.audioNodes.push(src, flt);
}
...
function _wave(dest) {
  ...
  lfo.connect(lg); lg.connect(g.gain); osc.connect(g); g.connect(dest);
  osc.start(); lfo.start();
...
function _forest(dest) {
  const ctx=state.audioContext;
  for(let i=0;i<3;i++){
    const o=ctx.createOscillator(); o.type='sine'; o.frequency.value=200+i*50;
...
window.startCryAnalysis = async function() {
  ...
  try {
    ...
    btn.disabled=false; btn.textContent=t('cryBtn');
  }
};

function _classifyCry(samples) {
  const aL=averageArray(samples.map(s=>s.low)),aM=averageArray(samples.map(s=>s.mid)),
        aH=averageArray(samples.map(s=>s.high)),aP=averageArray(samples.map(s=>s.peak));
